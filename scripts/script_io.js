...
sendButton.addEventListener("click", submitFunction);
...
message_to_send.addEventListener("keydown", (e) => {
    if (e.keyCode === 13) { 
        submitFunction();
    }
});
...
socket.on('chat message', (message) => {
    if (message.type === 'ordinary') {
        ...
        messageList.insertAdjacentHTML('beforeend', createBotMessageBody(message));
    }
...
socket.on('chat history', (messages) => { 
    for (let i = 0; i < messages.length; i++) {
        if(messages[i].type === 'bot')  messageList.insertAdjacentHTML('beforeend', createBotMessageBody(messages[i])); 
        else if (messages[i].type !== 'ordinary') messageList.insertAdjacentHTML('beforeend', createGeneralMessageBody(messages[i]));             
...
function createBotMessageBody(message) {
    const { user, body, time } = message;
    let elem = `<li class="clearfix"><div class="message-data align-right"><span class="message-data-time" >${time}</span> &nbsp; &nbsp;<span class="message-data-name" >${user.name}(${user.nickname})</span></div><div class="message bot">${body}</div></li>`;
    return elem;
}
