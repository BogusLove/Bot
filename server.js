...
const DateTools = require('./Entities/DateTools');
...
app.get('/io', (req, res) => {
    res.sendFile(__dirname + '/public/index_io.html');
})

app.get('/ajax', (req, res) => {
    res.sendFile(__dirname + '/public/index_ajax.html');
});

app.get('/script_io.js', (req, res) => {
    res.sendFile(__dirname + '/scripts/script_io.js');
});

app.get('/script_ajax.js', (req, res) => {
    res.sendFile(__dirname + '/scripts/script_ajax.js');
});
...
io.on('connection', (socket) => {
    ...
    socket.on('chat message', async (body) => {
        ...
        if (body.includes('@bot')) {
            ...
            try {
                let api = new BOT(body).apply();
                botMessage.body = await api.getResult();
            } catch (err){
                console.error(err);
                botMessage.body = 'Error: problems with connecting to @bot';
            }
            messages.push(botMessage);
            socket.broadcast.emit('chat message', botMessage);
            socket.emit('own message', botMessage);
        }
